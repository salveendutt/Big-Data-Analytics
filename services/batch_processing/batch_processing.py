...
def create_and_save_views(spark, cassandra_session):
    ...
    try:
        ...
        if df1 is not None:
            ...
            fraud_by_type = (
                ...
                .agg(
                    ...
                    sum(when(col("isFraud") == 1, 1).otherwise(0)).alias(
                        "total_fraudulent"
                    ),
                    avg("amount").alias("avg_amount"),
                )
                .withColumn(
                    "fraud_rate", col("total_fraudulent") / col("total_transactions")
                )
            )
            ...
            for row in fraud_by_type.collect():
                cassandra_session.execute(
                    ...
                    (
                        ...
                        row.total_fraudulent,
                        float(row.avg_amount),
                        float(row.fraud_rate),
                    ),
                )
        ...
        if df3 is not None:
            ...
            )  # Only customers with sufficient history
